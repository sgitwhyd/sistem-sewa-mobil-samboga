...
<script src="https://cdn.datatables.net/2.0.7/js/dataTables.bootstrap5.js"></script>
 ...
 <script>
  $(document).ready(function() {
    ...
    $('#tableBank tbody').on('click', '.view-detail', function () {
      var itemId = $(this).data('id');
      $.ajax({
          url: "<?= base_url('admin/bank/detail/'); ?>" + itemId,
          type: 'GET',
          success: function(data) {
              $('#detailBank').html('<dl class="row mt-2">\
              <dt class="col-sm-3">Nama Bank</dt>\
              <dd class="col-sm-9">'+ data['bank_name'] +'</dd>\
              <dt class="col-sm-3">No Rekening</dt>\
              <dd class="col-sm-9">'+ data['bank_number'] +'</dd>\
              <dt class="col-sm-3">a/n Rekening</dt>\
              <dd class="col-sm-9">'+ data['bank_owner'] +'</dd>\
            </dl>');
              $('#viewBank').modal('show');
          }
      });
